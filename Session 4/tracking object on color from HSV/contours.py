...
while True:
    ...
    contours, _ = cv2.findContours(myMaskComp, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    # cv2.drawContours(frame,contours, -1, (255,0,0), 3)
   
    for contour in contours:
        area = cv2.contourArea(contour)
        if area >= 500: # only keep large area. Removes noise
            # mycountour=[contour] # function expects an array of an array
            # cv2.drawContours(frame,[contour], 0, (0,255,255), 3)
            x,y,w,h = cv2.boundingRect(contour)
            cv2.rectangle(frame, (x,y), (x+w, y+h), (0,255,0),3) 
            cv2.putText(frame, str(x)+","+str(y), (x+int(w/2), y+int(h/2)), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 3, cv2.LINE_AA)
            print(x,y)
            # Wx = x
            # Wy = y
            Wx = int(Wx/width*1920)
            Wy = int(Wy/height*1080)

    cv2.imshow("my Webcam", frame)
    cv2.moveWindow("my Webcam", Wx,Wy)
